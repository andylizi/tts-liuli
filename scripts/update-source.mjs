...
const localLastUpdatedStr = (await $`git log -1 --pretty="format:%ct" -- ${LOCAL_FILE}`) || '0'
const localLastUpdated = new Date(+localLastUpdatedStr * 1000)

echo('Local version was updated on', localLastUpdated.toISOString())
...
echo('Remote version was updated on', remoteLastUpdated.toISOString())
...
await fs.writeFile(LOCAL_FILE, `# Downloaded from ${WORK_URL}\n\n${text}\n`)
